...
describe('useNpmTopByPopularity', () => {
  it('returns data with the default limit', async () => {
    mockTopByPopularity.mockResolvedValue(mockResult);

    const { result } = renderHook(() => useNpmTopByPopularity(), { wrapper });

    await waitFor(() => expect(result.current.isLoading).toBe(false));

    expect(result.current.data).toEqual(mockResult);
    expect(mockTopByPopularity).toHaveBeenCalledWith(20, expect.anything());
  });

  it('passes a custom limit to the client', async () => {
    mockTopByPopularity.mockResolvedValue(mockResult);

    const { result } = renderHook(() => useNpmTopByPopularity({ n: 5 }), { wrapper });

    await waitFor(() => expect(result.current.isLoading).toBe(false));

    expect(mockTopByPopularity).toHaveBeenCalledWith(5, expect.anything());
  });

  it('returns error on failure', async () => {
    mockTopByPopularity.mockRejectedValue(new NpmApiError(500, 'Internal Server Error'));
  ...
  it('does not fetch when enabled is false', () => {
    const { result } = renderHook(() => useNpmTopByPopularity({ enabled: false }), { wrapper });
  ...
  it('accepts queryOptions', async () => {
    mockTopByPopularity.mockResolvedValue(mockResult);
    const { result } = renderHook(() => useNpmTopByPopularity({ queryOptions: { staleTime: 0 } }), {
      wrapper,
    });
