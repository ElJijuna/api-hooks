...
describe('useNpmPackageLatest', () => {
  it('returns data on success', async () => {
    mockGet.mockResolvedValue(mockVersion);

    const { result } = renderHook(() => useNpmPackageLatest('react'), { wrapper });
  ...
  it('returns error on failure', async () => {
    ...
    const { result } = renderHook(() => useNpmPackageLatest('nonexistent-pkg-xyz'), { wrapper });
  ...
  it('does not fetch when name is empty', () => {
    const { result } = renderHook(() => useNpmPackageLatest(''), { wrapper });
  ...
  it('accepts queryOptions', async () => {
    ...
    const { result } = renderHook(
      () => useNpmPackageLatest('react', { queryOptions: { staleTime: 0 } }),
      { wrapper },
    );
