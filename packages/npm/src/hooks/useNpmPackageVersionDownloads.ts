...
import type { NpmVersionDownloadPeriod, NpmVersionDownloadPoint } from 'npmjs-api-client';
...
export interface UseNpmPackageVersionDownloadsOptions {
  ...
  enabled?: boolean;
  queryOptions?: QueryOverrides<NpmVersionDownloadPoint>;
}
...
export function useNpmPackageVersionDownloads(
  ...
  const { period = 'last-week', enabled = true, queryOptions } = options;
