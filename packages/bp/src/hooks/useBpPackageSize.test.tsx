...
describe('useBpPackageSize', () => {
  it('returns data on success', async () => {
    mockSize.mockResolvedValue(mockBundleSize);

    const { result } = renderHook(() => useBpPackageSize('react'), { wrapper });
    ...
    expect(mockSize).toHaveBeenCalledWith(undefined, expect.anything());
  });

  it('returns error on failure', async () => {
    mockSize.mockRejectedValue(new BundlephobiaApiError(404, 'Not Found'));

    const { result } = renderHook(() => useBpPackageSize('nonexistent-pkg-xyz'), { wrapper });
  ...
  it('does not fetch when name is empty', () => {
    const { result } = renderHook(() => useBpPackageSize(''), { wrapper });
