import { type UseQueryResult, useQuery } from '@tanstack/react-query';
import { pkggoQueryKeys } from '../keys/pkggoQueryKeys.js';
import { usePkgGoClient } from '../PkgGoClientContext.js';
import type { UsePkgGoQueryOptions } from './options.js';

/**
 * Downloads the module zip archive for a version.
 *
 * @param modulePath - Go module path (e.g. `'golang.org/x/mod'`)
 * @param version - Module version (e.g. `'v0.37.0'`)
 * @param options - Query options
 * @returns TanStack Query result with the zip archive bytes as an `ArrayBuffer`
 */
export function usePkgGoVersionZip(
  modulePath: string,
  version: string,
  options: UsePkgGoQueryOptions = {},
): UseQueryResult<ArrayBuffer, Error> {
  const { enabled = true, queryOptions } = options;
  const client = usePkgGoClient();

  return useQuery<ArrayBuffer, Error>({
    queryKey: pkggoQueryKeys.versionZip(modulePath, version),
    queryFn: ({ signal }) => client.module(modulePath).version(version).zip(signal),
    ...queryOptions,
    enabled: enabled && modulePath.length > 0 && version.length > 0,
  });
}
