...
beforeEach(() => {
  jest.clearAllMocks();
  jest.spyOn(GitHubClient.prototype, 'repo').mockReturnValue({ issues: mockIssues } as unknown as ReturnType<GitHubClient['repo']>);
});

const mockIssue = { id: 1, number: 1, title: 'Found a bug', state: 'open' } as unknown as GitHubIssue;
...
describe('useGhRepoIssuesInfinite', () => {
  it('fetches the first page on mount', async () => {
    ...
    const { result } = renderHook(() => useGhRepoIssuesInfinite('octocat', 'Hello-World'), { wrapper });
  ...
  it('fetches the next page when fetchNextPage is called', async () => {
    mockIssues.mockResolvedValueOnce(makeResponse(true, 2)).mockResolvedValueOnce(makeResponse(false));
    const { result } = renderHook(() => useGhRepoIssuesInfinite('octocat', 'Hello-World'), { wrapper });
    await waitFor(() => expect(result.current.isLoading).toBe(false));
  ...
  it('does not fetch when enabled is false', () => {
    const { result } = renderHook(() => useGhRepoIssuesInfinite('octocat', 'Hello-World', undefined, { enabled: false }), { wrapper });
