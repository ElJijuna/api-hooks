...
describe('useGhPullRequestCommits', () => {
  it('returns data on success', async () => {
    ...
    const { result } = renderHook(() => useGhPullRequestCommits('octocat', 'Hello-World', 42), { wrapper });
  ...
  it('passes params to the client', async () => {
    ...
    const { result } = renderHook(() => useGhPullRequestCommits('octocat', 'Hello-World', 42, params), { wrapper });
    await waitFor(() => expect(result.current.isLoading).toBe(false));
  ...
  it('does not fetch when enabled is false', () => {
    const { result } = renderHook(() => useGhPullRequestCommits('octocat', 'Hello-World', 42, undefined, { enabled: false }), { wrapper });
