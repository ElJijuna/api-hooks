...
function makeResponse(hasNextPage: boolean, nextPage?: number): GitHubPagedResponse<GitHubContributor> {
...
describe('useGhRepoContributorsInfinite', () => {
  it('fetches the first page on mount', async () => {
    ...
    const { result } = renderHook(() => useGhRepoContributorsInfinite('octocat', 'Hello-World'), { wrapper });
  ...
  it('fetches the next page when fetchNextPage is called', async () => {
    mockContributors.mockResolvedValueOnce(makeResponse(true, 2)).mockResolvedValueOnce(makeResponse(false));
    const { result } = renderHook(() => useGhRepoContributorsInfinite('octocat', 'Hello-World'), { wrapper });
    await waitFor(() => expect(result.current.isLoading).toBe(false));
  ...
  it('does not fetch when enabled is false', () => {
    const { result } = renderHook(() => useGhRepoContributorsInfinite('octocat', 'Hello-World', undefined, { enabled: false }), { wrapper });
