...
describe('useGhRepoWorkflows', () => {
  it('returns data on success', async () => {
    mockWorkflows.mockResolvedValue(mockResponse);
    const { result } = renderHook(() => useGhRepoWorkflows('owner', 'repo'), { wrapper });
  ...
  it('does not fetch when owner is empty', () => {
    const { result } = renderHook(() => useGhRepoWorkflows('', 'repo'), { wrapper });
    expect(result.current.isLoading).toBe(false);
    expect(mockWorkflows).not.toHaveBeenCalled();
  });

  it('does not fetch when repo is empty', () => {
    const { result } = renderHook(() => useGhRepoWorkflows('owner', ''), { wrapper });
    expect(result.current.isLoading).toBe(false);
    expect(mockWorkflows).not.toHaveBeenCalled();
  });
  ...
  it('accepts queryOptions', async () => {
    mockWorkflows.mockResolvedValue(mockResponse);
    const { result } = renderHook(
      () => useGhRepoWorkflows('owner', 'repo', { queryOptions: { staleTime: 0 } }),
      { wrapper },
    );
