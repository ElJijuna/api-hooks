...
const mockResponse: GitHubPagedResponse<GitHubIssueComment> = {
  values: [mockIssueComment],
  hasNextPage: false,
};
...
describe('useGhIssueComments', () => {
  it('returns data on success', async () => {
    ...
    const { result } = renderHook(() => useGhIssueComments('octocat', 'Hello-World', 1), {
      wrapper,
    });
  ...
  it('passes params to the client', async () => {
    ...
    const { result } = renderHook(() => useGhIssueComments('octocat', 'Hello-World', 1, params), {
      wrapper,
    });
    await waitFor(() => expect(result.current.isLoading).toBe(false));
  ...
  it('does not fetch when issueNumber is 0', () => {
    const { result } = renderHook(() => useGhIssueComments('octocat', 'Hello-World', 0), {
      wrapper,
    });
  ...
  it('does not fetch when enabled is false', () => {
    const { result } = renderHook(
      () => useGhIssueComments('octocat', 'Hello-World', 1, undefined, { enabled: false }),
      { wrapper },
    );
    expect(result.current.isLoading).toBe(false);
    expect(mockComments).not.toHaveBeenCalled();
