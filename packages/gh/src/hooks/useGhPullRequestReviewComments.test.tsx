...
import { GitHubClient, GitHubApiError, type GitHubReviewComment, type GitHubPagedResponse } from 'gh-api-client';
import { useGhPullRequestReviewComments } from './useGhPullRequestReviewComments.js';

const mockReviewComments = jest.fn<(params?: object, signal?: AbortSignal) => Promise<GitHubPagedResponse<GitHubReviewComment>>>();
...
const mockReviewComment = { id: 1, body: '', created_at: '', updated_at: '' } as unknown as GitHubReviewComment;
const mockResponse: GitHubPagedResponse<GitHubReviewComment> = { values: [mockReviewComment], hasNextPage: false };
...
describe('useGhPullRequestReviewComments', () => {
  it('returns data on success', async () => {
    ...
    const { result } = renderHook(() => useGhPullRequestReviewComments('octocat', 'Hello-World', 42), { wrapper });
  ...
  it('passes params to the client', async () => {
    ...
    const { result } = renderHook(() => useGhPullRequestReviewComments('octocat', 'Hello-World', 42, params), { wrapper });
    await waitFor(() => expect(result.current.isLoading).toBe(false));
  ...
  it('does not fetch when enabled is false', () => {
    const { result } = renderHook(() => useGhPullRequestReviewComments('octocat', 'Hello-World', 42, undefined, { enabled: false }), { wrapper });
