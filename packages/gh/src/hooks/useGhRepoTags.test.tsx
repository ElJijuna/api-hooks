...
describe('useGhRepoTags', () => {
  ...
  it('passes params to the client', async () => {
    ...
    const { result } = renderHook(() => useGhRepoTags('octocat', 'Hello-World', params), {
      wrapper,
    });
    await waitFor(() => expect(result.current.isLoading).toBe(false));
  ...
  it('does not fetch when enabled is false', () => {
    const { result } = renderHook(
      () => useGhRepoTags('octocat', 'Hello-World', undefined, { enabled: false }),
      { wrapper },
    );
  ...
  it('accepts queryOptions', async () => {
    mockTags.mockResolvedValue(mockResponse);
    const { result } = renderHook(
      () => useGhRepoTags('octocat', 'Hello-World', { queryOptions: { staleTime: 0 } }),
      { wrapper },
    );
