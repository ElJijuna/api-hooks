...
const mockForks = jest.fn<(params?: object, signal?: AbortSignal) => Promise<GitHubPagedResponse<GitHubRepository>>>();

beforeEach(() => {
  jest.clearAllMocks();
  jest.spyOn(GitHubClient.prototype, 'repo').mockReturnValue({ forks: mockForks } as unknown as ReturnType<GitHubClient['repo']>);
});
...
describe('useGhRepoForksInfinite', () => {
  it('fetches the first page on mount', async () => {
    ...
    const { result } = renderHook(() => useGhRepoForksInfinite('octocat', 'Hello-World'), { wrapper });
  ...
  it('fetches the next page when fetchNextPage is called', async () => {
    mockForks.mockResolvedValueOnce(makeResponse(true, 2)).mockResolvedValueOnce(makeResponse(false));
    const { result } = renderHook(() => useGhRepoForksInfinite('octocat', 'Hello-World'), { wrapper });
    await waitFor(() => expect(result.current.isLoading).toBe(false));
  ...
  it('does not fetch when enabled is false', () => {
    const { result } = renderHook(() => useGhRepoForksInfinite('octocat', 'Hello-World', undefined, { enabled: false }), { wrapper });
