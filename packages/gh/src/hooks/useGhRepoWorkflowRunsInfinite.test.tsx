...
import { GitHubClient, GitHubApiError, type GitHubWorkflowRunsResponse } from 'gh-api-client';
import { useGhRepoWorkflowRunsInfinite } from './useGhRepoWorkflowRunsInfinite.js';

const mockWorkflowRuns = jest.fn<(params?: object, signal?: AbortSignal) => Promise<GitHubWorkflowRunsResponse>>();
...
const mockRun = { id: 1, name: 'CI', run_number: 42, status: 'completed', conclusion: 'success' } as unknown as GitHubWorkflowRunsResponse['workflow_runs'][0];
...
describe('useGhRepoWorkflowRunsInfinite', () => {
  it('returns data on success', async () => {
    ...
    const { result } = renderHook(() => useGhRepoWorkflowRunsInfinite('octocat', 'Hello-World'), { wrapper });
  ...
  it('passes params to the client', async () => {
    ...
    const { result } = renderHook(() => useGhRepoWorkflowRunsInfinite('octocat', 'Hello-World', params), { wrapper });
    await waitFor(() => expect(result.current.isLoading).toBe(false));
  ...
  it('does not fetch when owner is empty', () => {
    const { result } = renderHook(() => useGhRepoWorkflowRunsInfinite('', 'Hello-World'), { wrapper });
