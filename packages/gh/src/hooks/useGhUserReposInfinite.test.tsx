...
import { useGhUserReposInfinite } from './useGhUserReposInfinite.js';

const mockRepos = jest.fn<(params?: object, signal?: AbortSignal) => Promise<GitHubPagedResponse<GitHubRepository>>>();

beforeEach(() => {
  jest.clearAllMocks();
  jest.spyOn(GitHubClient.prototype, 'user').mockReturnValue({ repos: mockRepos } as unknown as ReturnType<GitHubClient['user']>);
});
...
describe('useGhUserReposInfinite', () => {
  it('fetches the first page on mount', async () => {
    mockRepos.mockResolvedValue(makeResponse(false));
    const { result } = renderHook(() => useGhUserReposInfinite('octocat'), { wrapper });
    await waitFor(() => expect(result.current.isLoading).toBe(false));
    expect(result.current.data?.pages).toHaveLength(1);
    expect(mockRepos).toHaveBeenCalledWith({ page: 1 }, expect.anything());
  });
  ...
  it('fetches the next page when fetchNextPage is called', async () => {
    ...
    expect(mockRepos).toHaveBeenNthCalledWith(2, { page: 2 }, expect.anything());
  });
  ...
  it('does not fetch when login is empty', () => {
    const { result } = renderHook(() => useGhUserReposInfinite(''), { wrapper });
    expect(result.current.isLoading).toBe(false);
    expect(mockRepos).not.toHaveBeenCalled();
  });

  it('does not fetch when enabled is false', () => {
    const { result } = renderHook(() => useGhUserReposInfinite('octocat', undefined, { enabled: false }), { wrapper });
    expect(result.current.isLoading).toBe(false);
    expect(mockRepos).not.toHaveBeenCalled();
  });
});
