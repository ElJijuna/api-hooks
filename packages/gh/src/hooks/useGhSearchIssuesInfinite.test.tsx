...
describe('useGhSearchIssuesInfinite', () => {
  it('returns data on success', async () => {
    ...
    const { result } = renderHook(() => useGhSearchIssuesInfinite({ q: 'is:issue is:open' }), { wrapper });
  ...
  it('does not fetch when enabled is false', () => {
    const { result } = renderHook(() => useGhSearchIssuesInfinite({ q: 'is:issue' }, { enabled: false }), { wrapper });
    expect(result.current.isLoading).toBe(false);
    expect(mockSearchIssues).not.toHaveBeenCalled();
  });
});
