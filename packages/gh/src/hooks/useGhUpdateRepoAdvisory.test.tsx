...
const mockUpdateAdvisory =
  jest.fn<(ghsaId: string, data: object) => Promise<GitHubRepositoryAdvisory>>();
...
describe('useGhUpdateRepoAdvisory', () => {
  ...
  it('accepts mutationOptions', async () => {
    ...
    const { result } = renderHook(
      () => useGhUpdateRepoAdvisory('owner', 'repo', { mutationOptions: { onSuccess } }),
      { wrapper },
    );
    act(() => {
      result.current.mutate({
