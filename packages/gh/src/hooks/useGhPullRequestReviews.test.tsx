...
import { GitHubClient, GitHubApiError, type GitHubReview, type GitHubPagedResponse } from 'gh-api-client';
import { useGhPullRequestReviews } from './useGhPullRequestReviews.js';

const mockReviews = jest.fn<(params?: object, signal?: AbortSignal) => Promise<GitHubPagedResponse<GitHubReview>>>();
...
const mockReview = { id: 1, body: '', state: 'APPROVED', submitted_at: '', commit_id: '' } as unknown as GitHubReview;
const mockResponse: GitHubPagedResponse<GitHubReview> = { values: [mockReview], hasNextPage: false };
...
describe('useGhPullRequestReviews', () => {
  it('returns data on success', async () => {
    ...
    const { result } = renderHook(() => useGhPullRequestReviews('octocat', 'Hello-World', 42), { wrapper });
  ...
  it('passes params to the client', async () => {
    ...
    const { result } = renderHook(() => useGhPullRequestReviews('octocat', 'Hello-World', 42, params), { wrapper });
    await waitFor(() => expect(result.current.isLoading).toBe(false));
  ...
  it('does not fetch when enabled is false', () => {
    const { result } = renderHook(() => useGhPullRequestReviews('octocat', 'Hello-World', 42, undefined, { enabled: false }), { wrapper });
